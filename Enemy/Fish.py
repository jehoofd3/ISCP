...
class Fish(Enemy):
    ...
    def __init__(self, x, y, range, type):
        ...
        if type == "Water":
            ...
            # These methods get them from the database and add them
            # to the image lists.
        ...
        # so its required to call the super class by using the
        # super() method.
        super(Fish, self).__init__(x, y, range, self.w_l, self.w_r, self.d_l,
                                   self.d_r, self.OB, self.LR)

        # When the fish is created this line of code changes the states
        # object in enemy into FishNormalState.
