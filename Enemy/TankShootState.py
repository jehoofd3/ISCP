...
class TankShootState(EnemyState):
    ...
    def __init__(self, enemy):
        ...
        # Multiple the speed of the tank by 1.2, so the tank moves faster when,
    ...
    def update(self):
        ...
        # Set the x speed of the tank to zero if he touches a tile on the left,
        # or right side and let him jump.
        # This lets the tank jump over tile.
        if self.enemy.block_l or self.enemy.block_r:
            self.enemy.x_speed = 0
            self.enemy.jump()

        # If the boolean shoot_first is True, shoot a bullet, and change the,
        # variable to False.
        # This makes the tank shoot automatically on the start of this state.
        if self.shoot_first:
            self.shoot()
        ...
        # than switch_time is shoots the bullet.
        # This makes it shoot every two seconds.
        if time.time() >= self.start_time + self.shoot_time:
            self.shoot()

        # If the boolean follow in the enemy class is Talse, change the state,
        # back to TankNormalState.
        # This boolean is changed by the collider and lets the tank unfollow,
        # the player.
